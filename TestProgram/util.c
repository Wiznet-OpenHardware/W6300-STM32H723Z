...
uint8_t is_ipaddr(uint8_t *src_ip, uint8_t *ret_ip)
{
  uint8_t i = 0;
  uint8_t dotcnt = 0;
  uint16_t tval = 0;
  uint8_t len = strlen((char *)src_ip);
  ...
  for (i = 0; i < len; i++)
  {
    ...
    else if (src_ip[i] == '.')
    {
      ...
      // added for ret_ip arrary
      ret_ip[dotcnt - 1] = atoi((const char *)tmp);
  ...
  // added for ret_ip arrary
  ret_ip[dotcnt] = atoi((const char *)tmp);
